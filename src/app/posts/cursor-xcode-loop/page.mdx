...
I've been developing my own iOS app, [Beam](https://trybeam.vercel.app), a project and task manager, in **Cursor**, my editor of choice. The agent writes Swift fine; verifying it meant constantly switching between Cursor and **Xcode**: Run, copy errors, paste, repeat. Tedious, low-value work.

That's the manual back-and-forth this post is about fixing. Here's how the **agentic loop** I wired up while building it works, plus a starter kit so you can use it on your own iOS project too.

As a web developer, I'm used to closing that loop with runtime feedback: test output, a dev server log, a failing API call. iOS is different: the gate is **compile time**. Swift has to build before anything runs, and that feedback lives in **Xcode** (builds, simulators, compiler errors). Cursor can write the Swift; it cannot see whether it compiled until something bridges the two.
