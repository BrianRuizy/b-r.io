    ...
    <header className="sticky top-0 z-10 main-header backdrop-blur-md bg-primary animate-in-reverse md:bg-header">
      <nav className="px-4 md:px-6 py-3 lg max-w-[700px] mx-auto flex justify-between items-center gap-3">
        ...
        <Popover className="relative ml-auto md:hidden">
          ...
          >
            <Popover.Panel className="absolute z-10 right-0 p-2 mt-2 overflow-auto text-base origin-top-right shadow-lg w-40 rounded-xl bg-blur backdrop-blur-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              <div className="grid">
                { links.map((link) => (
                  <Link 
                    key={link.href} 
