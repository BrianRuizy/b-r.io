---
title: 'Tailwind CSS over everything. Mocking iOS weather UI'
...
Check out this iOS weather widget. I was able to create this component in less than 11 minutes, which would have taken me much longer if I had to write the CSS from scratch.
...
In design, there's `User Interface`, `User Experience`, `Human Interface` (as Apple likes to call it), but let's not forget about `Developer Experience`. Tailwind is a great example of a tool that makes the developer experience better.

- Don't spend time thinking of funky names <sup>1</sup>
...
One of the key advantages of Tailwind CSS is its promotion of reusable components. By using pre-defined classes, Tailwind allows developers to create reusable components that can be easily used throughout the codebase. 

Continuing with the weather theme here, let's create a reusable card component that can be used to display the current weather for a list of cities. 
...
<div className="flex flex-col gap-2">
  {data.map((item) => (
    ...
    `}>
      <div>
        <p className="font-bold text-xl leading-tight">{item.city}</p>
        <p className="text-sm font-medium mb-5 opacity-75 leading-tight">{item.time}</p>
